...
{
	// Check if we are turning an element on or off
	if(toggle == 0)
	{
		// If we are turning the element off, hide visibility and set height to 0
		document.getElementById(id).style.visibility = "hidden";
		document.getElementById(id).style.height = "0";
	}
	else
	{
		// If we are turning the element on, make visible and set height to auto
...
{
	// Save the stats table as a variable for easy referencing
	var table = document.getElementById("stats_table");

	// Get the number of rows for iterating
	var numRows = table.rows.length;
	var wins = 0;
	var losses = 0;

	// Start on the third row because the first 2 are the heading information
	for(i = 2;i < numRows;i++)
	{
		// Check if the home score (col 2) or opp. score (col 3) is higher
		if(parseInt(table.rows[i].cells[2].innerHTML) > parseInt(table.rows[i].cells[3].innerHTML))
		{
			// If home score is higher, set "CU Boulder" to the winner (col 4)
			table.rows[i].cells[4].innerHTML = "CU Boulder";
			wins++;
		}
		else
		{
			// If home score is lower, set "Opposing Team Name" (col 1) as winner
			table.rows[i].cells[4].innerHTML = table.rows[i].cells[1].innerHTML;
			losses++;
		}
	}

	// Assign win and loss counts
...
{
	// Create a variable for the dropdown menu for easy reference
	var dropdown = document.getElementById("player_selector");

	// Loop through player array
	for(i = 0;i < players.length;i++)
	{
		// Create a new anchor, fill its elements, and append it to the dropdown list
		var newAnchor = document.createElement('a');
		newAnchor.href = "#";
		newAnchor.className = "dropdown-item";
		newAnchor.onclick = switchPlayers(i);
...
{
	// Update spans on page with player information
	...
	// Set the average values on the page
